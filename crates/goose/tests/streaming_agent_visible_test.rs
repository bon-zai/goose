...
use goose::providers::base::{Provider, ProviderUsage, Usage, MessageStream};
...
async fn test_streaming_filters_agent_invisible_messages() {
    ...
    assert_eq!(captured[1].as_concat_text(), "This is visible to agent only");
...
async fn test_complete_filters_agent_invisible_messages() {
    ...
    assert_eq!(captured.len(), 3, "Should filter out non-agent-visible messages");
